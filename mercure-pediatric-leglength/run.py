...
def validate_config(config: dict, log: logging.Logger) -> list:
    ...
    # Load valid models dynamically from registry.json
    try:
        import json
        import os
        ...
        log.info(f"Loaded {len(valid_models)} models from registry: {valid_models}")
    except Exception as e:
        log.warning(f"Could not load registry.json, using default models: {e}")
        valid_models = [
