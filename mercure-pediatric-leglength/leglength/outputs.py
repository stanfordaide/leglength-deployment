...
class DicomProcessor:
    ...
    # AI disclaimer for DICOM SC output (not in DICOM SR)
    ...
    def _create_visualization_image(self, results: dict, dicom_path: str) -> np.ndarray:
        ...
        if original_image.min() == original_image.max():
            ...
            image_region = cv2.cvtColor(test_img, cv2.COLOR_GRAY2BGR)
        else:
            ...
            image_region = cv2.cvtColor(enhanced_image, cv2.COLOR_GRAY2BGR)
            # Add subtle blue tint for medical aesthetic
            image_region[:,:,0] = (image_region[:,:,0] * 0.95).astype(np.uint8)  # Reduce red
            image_region[:,:,1] = (image_region[:,:,1] * 0.98).astype(np.uint8)  # Slightly reduce green
    ...
    def _create_measurements_container(self, content_seq: List[Dataset], results: dict, femur_threshold: float = 2.0, tibia_threshold: float = 2.0, total_threshold: float = 5.0) -> Dataset:
        ...
        # Round the 4 base measurements to 1dp once; every display and calculation below uses
        # these values so that arithmetic stays consistent and float subtraction artefacts
        # (e.g. round(46.3,1) - round(46.2,1) == 0.0999...) cannot creep in.
        meas = results['measurements']
        r_fem = round(meas['PLL_R_FEM']['centimeters'], 1) if 'PLL_R_FEM' in meas else None
        l_fem = round(meas['PLL_L_FEM']['centimeters'], 1) if 'PLL_L_FEM' in meas else None
        r_tib = round(meas['PLL_R_TIB']['centimeters'], 1) if 'PLL_R_TIB' in meas else None
        l_tib = round(meas['PLL_L_TIB']['centimeters'], 1) if 'PLL_L_TIB' in meas else None

        # Create separate content items for each measurement
        for name, data in meas.items():
            text_item = Dataset()
            ...
            if name == 'PLL_R_FEM':
                ...
                text = f"{round(r_fem + r_tib, 1):.1f}" if r_fem is not None and r_tib is not None else f"{data['centimeters']:.1f}"
            elif name == 'PLL_L_FEM':
                ...
                text = f"{round(l_fem + l_tib, 1):.1f}" if l_fem is not None and l_tib is not None else f"{data['centimeters']:.1f}"
            else:
        ...
        if r_fem is not None and l_fem is not None:
            fem_diff = round(r_fem - l_fem, 1)
        ...
        if r_tib is not None and l_tib is not None:
            tib_diff = round(r_tib - l_tib, 1)
        ...
        if all(v is not None for v in [r_fem, l_fem, r_tib, l_tib]):
            total_diff = round(r_fem + r_tib - l_fem - l_tib, 1)
    ...
    def get_qa_dicom(self, results: dict, dicom_path: str) -> Dataset:
        ...
        # Create visualization image (in BGR format from OpenCV)
        visualization = self._create_visualization_image(results, dicom_path)
        
        # Convert BGR to RGB for DICOM (DICOM RGB expects RGB order, not BGR)
        # This fixes the color inversion issue in PACS viewers
        ...
        # Set pixel data and image dimensions (using RGB converted image)
        new_ds.PixelData = visualization_rgb.tobytes()
        new_ds.Rows = visualization_rgb.shape[0]
        new_ds.Columns = visualization_rgb.shape[1]
    ...
    def _create_model_coverage_table(self, results: dict, models: List[str]) -> np.ndarray:
        ...
        # ALWAYS show all 8 anatomical points (P1-P8) for complete QA coverage
        # This ensures missing points are visible in the table as ✗ across all models
        point_ids = list(range(8))  # 0-7 for P1-P8
        
        # Log which points were actually detected for debugging
        detected_points = set()
        for model_name, predictions in model_predictions.items():
            if 'predictions' in predictions and 'labels' in predictions['predictions']:
                ...
                for label in labels:
                    if 1 <= label <= 8:  # Valid anatomical point labels
                        detected_points.add(label - 1)  # Convert to 0-based
        
        self.logger.info(f"All points (P1-P8): showing all 8 anatomical landmarks")
        self.logger.info(f"Points detected by at least one model: {sorted([p+1 for p in detected_points])}")
        missing_points = set(point_ids) - detected_points
        if missing_points:
            self.logger.warning(f"Points NOT detected by ANY model: {sorted([p+1 for p in missing_points])}")
    ...
    def _create_combined_qa_table_image(self, results: dict, dicom_path: str, models: List[str]) -> np.ndarray:
        ...
        # Dynamically create legend from actual models used
        model_legend_parts = [f"m{i+1}={models[i]}" for i in range(len(models))]
        legend_text = "Legend: " + " | ".join(model_legend_parts)
    ...
    def get_qa_table_dicom(self, results: dict, dicom_path: str, models: List[str]) -> Dataset:
        ...
        # Create combined visualization image (in BGR format from OpenCV)
