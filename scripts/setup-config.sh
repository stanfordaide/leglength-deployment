...
# Parse DICOM modalities from config.env and extract components
# Format: MODALITY_NAME=AET|HOST|PORT
parse_modalities() {
    local modality_var="$1"
    local aet=$(echo "$modality_var" | cut -d'|' -f1)
    local host=$(echo "$modality_var" | cut -d'|' -f2)
    local port=$(echo "$modality_var" | cut -d'|' -f3)
    echo "$aet|$host|$port"
}

# Extract modality components for template substitution
MODALITY_MERCURE_PARSED=$(parse_modalities "$MODALITY_MERCURE")
MODALITY_MERCURE_AET=$(echo "$MODALITY_MERCURE_PARSED" | cut -d'|' -f1)
MODALITY_MERCURE_HOST=$(echo "$MODALITY_MERCURE_PARSED" | cut -d'|' -f2)
MODALITY_MERCURE_PORT=$(echo "$MODALITY_MERCURE_PARSED" | cut -d'|' -f3)

MODALITY_LPCHROUTER_PARSED=$(parse_modalities "$MODALITY_LPCHROUTER")
MODALITY_LPCHROUTER_AET=$(echo "$MODALITY_LPCHROUTER_PARSED" | cut -d'|' -f1)
MODALITY_LPCHROUTER_HOST=$(echo "$MODALITY_LPCHROUTER_PARSED" | cut -d'|' -f2)
MODALITY_LPCHROUTER_PORT=$(echo "$MODALITY_LPCHROUTER_PARSED" | cut -d'|' -f3)

MODALITY_LPCHTROUTER_PARSED=$(parse_modalities "$MODALITY_LPCHTROUTER")
MODALITY_LPCHTROUTER_AET=$(echo "$MODALITY_LPCHTROUTER_PARSED" | cut -d'|' -f1)
MODALITY_LPCHTROUTER_HOST=$(echo "$MODALITY_LPCHTROUTER_PARSED" | cut -d'|' -f2)
MODALITY_LPCHTROUTER_PORT=$(echo "$MODALITY_LPCHTROUTER_PARSED" | cut -d'|' -f3)

MODALITY_MODLINK_PARSED=$(parse_modalities "$MODALITY_MODLINK")
MODALITY_MODLINK_AET=$(echo "$MODALITY_MODLINK_PARSED" | cut -d'|' -f1)
MODALITY_MODLINK_HOST=$(echo "$MODALITY_MODLINK_PARSED" | cut -d'|' -f2)
MODALITY_MODLINK_PORT=$(echo "$MODALITY_MODLINK_PARSED" | cut -d'|' -f3)

# Export for envsubst
export MODALITY_MERCURE_AET MODALITY_MERCURE_HOST MODALITY_MERCURE_PORT
export MODALITY_LPCHROUTER_AET MODALITY_LPCHROUTER_HOST MODALITY_LPCHROUTER_PORT
export MODALITY_LPCHTROUTER_AET MODALITY_LPCHTROUTER_HOST MODALITY_LPCHTROUTER_PORT
export MODALITY_MODLINK_AET MODALITY_MODLINK_HOST MODALITY_MODLINK_PORT

envsubst '${ORTHANC_AET} ${ORTHANC_DB_USER} ${ORTHANC_DB_PASS} ${ORTHANC_ADMIN_USER} ${ORTHANC_ADMIN_PASS} ${MODALITY_MERCURE_AET} ${MODALITY_MERCURE_HOST} ${MODALITY_MERCURE_PORT} ${MODALITY_LPCHROUTER_AET} ${MODALITY_LPCHROUTER_HOST} ${MODALITY_LPCHROUTER_PORT} ${MODALITY_LPCHTROUTER_AET} ${MODALITY_LPCHTROUTER_HOST} ${MODALITY_LPCHTROUTER_PORT} ${MODALITY_MODLINK_AET} ${MODALITY_MODLINK_HOST} ${MODALITY_MODLINK_PORT}' \
