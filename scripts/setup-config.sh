...
export MODALITY_LPCHROUTER104_JSON=$(format_modality "$MODALITY_LPCHROUTER104")
...
export MODALITY_MODLINKTEST_JSON=$(format_modality "$MODALITY_MODLINKTEST")

envsubst '${ORTHANC_AET} ${ORTHANC_DB_USER} ${ORTHANC_DB_PASS} ${ORTHANC_ADMIN_USER} ${ORTHANC_ADMIN_PASS} ${MODALITY_MERCURE_JSON} ${MODALITY_LPCHROUTER_JSON} ${MODALITY_LPCHROUTER104_JSON} ${MODALITY_LPCHTROUTER_JSON} ${MODALITY_MODLINK_JSON} ${MODALITY_MODLINKTEST_JSON}' \
...
envsubst '${MONITORING_DATA_PATH} ${GRAPHITE_IP} ${GRAPHITE_PORT} ${MONITORING_DB_HOST} ${MONITORING_DB_NAME} ${MONITORING_DB_USER} ${MONITORING_DB_PASS} ${MONITORING_DB_PORT} ${ORTHANC_ADMIN_USER} ${ORTHANC_ADMIN_PASS}' \
